...
module Eval (M : MONADERROR) = struct
  ...
  let ht_find_opt_and_process ht key f emsg =
    match Ast.Hashtbl.find_opt ht key with
    | Some v -> f v
    | None -> error emsg
  ;;
  ...
  let rec take_after n l =
    match l with
    | _ :: tl when n > 0 -> take_after (n - 1) tl
    | _ -> l
  ;;
  ...
  let get_from_struct_tags ctx tag =
    get_from_vars ctx tag "Struct undefined"
    >>= fun _ -> return @@ Ast.Hashtbl.find_opt ctx.struct_tags tag
  ;;
  ...
  let rec sizeof ctx t =
    ...
    | CT_STRUCT tag ->
      ...
      | Some ps ->
        List.fold_left
          (fun acc (tt, _) ->
            acc >>= fun ac -> sizeof ctx tt >>= fun inc -> return @@ (ac + inc))
          (return 0)
          ps
        >>= fun x -> get_size @@ CT_STRUCT "tag" >>= fun inc -> return @@ (x + inc)
  ...
  let create_var ctx name (v : v_value) t =
    ...
    | Vhval (Vrval (_n, _v)) ->
      let ctx = ctx in
      add_in_vars ctx name ctx.free_addr (Vrval (_n, _v)) t
      >>= fun ctx -> add_in_heap ctx ctx.free_addr (Vrval (_n, _v)) t
    | _ -> error "create_var"
  ;;
  ...
  let add_in_functions ctx name (typ, args, sts) =
    let add_in_functions' ctx name (vrs, typ, args, sts) =
      create_var ctx name (Vhval (Vrval (name, Vglob (0, Vfuncdec name)))) CT_VOID
      >>= fun ctx' ->
  ...
  let rec conv_to_addr ctx tt = function
    ...
    | Vstructval (_, Vaddress (_, v)) | Vaddress (_, v) -> return @@ Vaddress (tt, v)
  ...
  let rec conv_to_char ctx = function
    | Vint v | Vaddress (_, v) -> return @@ Vchar (String.get (Int.to_string v) 0)
  ...
  let rec conv_v ctx v _v =
    ...
    | Vstructval (n, v') -> conv_v ctx v v' >>= fun v'' -> return @@ Vstructval (n, v'')
  ...
  let rec eval_stmt
    ...
    | IF_ELSE (e, if_blk, else_blk) ->
      ...
      in
      conv_to_int ct @@ get_val bv
      >>= fun x ->
      (if get_int x > 0
      then eval_ifelse @@ stmts if_blk
      else (
    ...
    | IF (e, blk) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ IF_ELSE (e, blk, BLOCK [])
    ...
    | ASSIGN_SUB (le, re) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ ASSIGN (le, SUB (le, re))
    | ASSIGN_ADD (le, re) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ ASSIGN (le, ADD (le, re))
    | ASSIGN_MUL (le, re) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ ASSIGN (le, MUL (le, re))
    | ASSIGN_DIV (le, re) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ ASSIGN (le, DIV (le, re))
    | ASSIGN_MOD (le, re) ->
      eval_stmt ctx from_main in_fn rwrt convt cur_t palcs @@ ASSIGN (le, MOD (le, re))
    | FOR (d, be, e, blk) ->
      let vars' : vars = Hashtbl.create 16 in
      Hashtbl.iter (fun n v -> Hashtbl.add vars' n v) ctx.vars;
      (match d with
      | Some dd ->
        eval_stmt { ctx with vars = vars' } from_main in_fn true convt cur_t palcs dd
      ...
      (let get_stmts = function
       ...
       | Some bbe ->
         ...
         | None -> eval_stmt c from_main in_fn false convt cur_t pals @@ WHILE (bbe, blk))
       | None ->
         (match e with
         | Some ee ->
           eval_stmt c from_main in_fn false convt cur_t pals
      ...
      >>= fun (c, als) ->
      Hashtbl.iter
        (fun n (t, a, hv) -> Hashtbl.replace ctx.vars n (t, a, get_from_heap c a))
        ctx.vars;
      return ({ ctx with free_addr = c.free_addr }, als)

  and eval_expr (ctxs : exec_ctx) from_main (in_fn : bool) convt cur_t (palcs : allocated)
    = function
    | VAR_NAME name ->
      get_from_vars ctxs name "Var undefined"
      >>= fun (t, _, v) -> return @@ (({ ctxs with cur_t = t }, Vhval v), palcs)
    ...
    | NOT e ->
      eval_expr ctxs from_main in_fn convt cur_t palcs e
      >>= fun ((cts, v), pls) -> _not cts v >>= fun v' -> return ((cts, v'), pls)
    | INDEXER (n, e) ->
      let xable ctxs name i =
        ...
        | CT_PTR t, _, Vrval (_, Vaddress (_, addr)) ->
          sizeof ctxs t
          >>= fun s -> return @@ (Vhval (get_from_heap ctxs (addr + (i * s))), cur_t)
      ...
      conv_to_int cts i
      >>= fun i' ->
      xable cts n @@ get_int i'
      >>= fun (v', cur_t) -> return (({ cts with cur_t }, v'), palcs)
    | FUNC_CALL (n, vals) ->
      (match n with
      | "malloc" ->
        (match vals with
    ...
    | ACCESOR (e1, e2) ->
      let strct_padding tag n bgn i =
        ...
        | Some args ->
          let rec pdng n args acc =
            ...
            | (tt, _) :: tl ->
              pdng n tl (acc >>= fun ac -> sizeof ctxs tt >>= fun x -> return @@ (x + ac))
            | [] -> error "Nonexistent struct field"
          in
          pdng n args (return 0)
          >>= fun p ->
          (match get_from_heap ctxs (bgn + p) with
          | Vdval (_, Vstructval (_, Varraddress (t', a'))) ->
            get_size t'
            >>= fun inc -> return @@ Vhval (get_from_heap ctxs (a' + (i * inc)))
          | v -> return @@ Vhval v)
        | None -> error "Struct undefined"
      in
      ...
      | (c, Vstructval (_, Vstaddress (tag, a))), pal
        ...
        (match e2 with
        | VAR_NAME n -> strct_padding tag n a 0 >>= fun v -> return ((ctxs, v), pal)
        | INDEXER (n, e) ->
          eval_expr ctxs from_main in_fn convt cur_t palcs e
          >>= fun ((_, i), pal) ->
          strct_padding tag n a @@ get_int i >>= fun v -> return ((ctxs, v), pal)
        | _ -> error "~~")
    ...
    | DEREFERENCE e ->
      ...
      | Vaddress (pt_t, _)
      | Varraddress (pt_t, _) -> return (pt_t, v)
      | _ -> error @@ "Can't be dereferenced")
      >>= fun (pt_t, v) ->
      conv_to_int cs @@ get_val v
      >>= fun v' ->
    ...
    | ADDRESS e ->
      ...
      (match v with
      | Vhval (Vrval (n, _)) ->
        get_from_vars ctxs n "Var undefined"
        >>= fun (_, a, _) -> return ((cs, Vint a), pals)
      | _ -> error @@ "~~")
  ...
  and repeat ctx from_main in_fn convt cur_t palcs body tmp_b cond =
    ...
    >>= fun x ->
    if x
    then eval_cyc_body c from_main in_fn convt cur_t pals cond body tmp_b
  ...
  and eval_cycle ctx from_main in_fn convt cur_t palcs cond = function
    | BLOCK stmts ->
      (let vars' : vars = Hashtbl.create 16 in
       Hashtbl.iter (fun n v -> Hashtbl.add vars' n v) ctx.vars;
       repeat { ctx with vars = vars' } from_main in_fn convt cur_t palcs stmts stmts cond)
      >>= fun (c, als) ->
      Hashtbl.iter
        (fun n (t, a, hv) -> Hashtbl.replace ctx.vars n (t, a, get_from_heap c a))
        ctx.vars;
  ...
  and main ctxs convt (cur_t : types) palcs =
    ...
    in
    let ctxs =
      { ctxs with vars = Hashtbl.create 16; last_value = Vvoid; have_ret = false }
    in
    stmts
    >>= fun (vrrrrs, _, _, vs) ->
    Hashtbl.iter (fun k v -> Hashtbl.add ctxs.vars k v) vrrrrs;
    blk ctxs false palcs vs
    >>= fun (ctxs', pls) ->
  ...
  and malloc ctxs (in_fn : bool) (palcs : allocated) size tt =
    let s' =
      sizeof ctxs tt >>= fun sot -> if size >= sot then return size else return sot
    in
    ...
    sizeof ctxs tt
    >>= fun sot ->
    cast_default_init0 ctxs @@ CT_ARRAY (s / sot, tt)
    >>= function
    | Vvalues vs ->
      List.fold_left
        (fun c v ->
          ...
          | Vstructval (n, v') ->
            ...
            >>= fun t ->
            c
            >>= fun c -> cast_default_dep_v c t n (CT_STRUCT "tag") >>= fun x -> return x
          | _ ->
            to_type ctxs v
            >>= fun t ->
            c
            >>= fun c ->
            cast_default_dep_v c t "" (CT_ARRAY (0, CT_INT)) >>= fun x -> return x)
        (return @@ ctxs)
        vs
      >>= fun x ->
      return @@ x
      >>= fun h -> return ((h, Vaddress (tt, adr)), (adr, adr + s - 1) :: palcs)
    (*  never come *)
  ...
  and cast_default_dep_v ctxs t n format =
    ...
    | CT_STRUCT tag ->
      ...
      | CT_INT | CT_CHAR | CT_VOID | CT_PTR _ ->
        ...
        >>= fun v ->
        add_in_heap ctx ctx.free_addr (Vdval (ctx.free_addr, Vstructval (n, v))) t
      | CT_STRUCT tag' ->
        get_size t
        >>= fun l ->
        add_in_heap
          ctx
          ctx.free_addr
          (Vdval (ctx.free_addr, Vstructval (n, Vstaddress (tag', ctx.free_addr + l))))
          t
      | CT_ARRAY (_, t') ->
        get_size t
        >>= fun l ->
        add_in_heap
          ctx
          ctx.free_addr
          (Vdval (ctx.free_addr, Vstructval (n, Varraddress (t', ctx.free_addr + l))))
          t)
    | _ ->
      (match t with
      | CT_INT | CT_CHAR | CT_VOID ->
        cast_default_init0 ctxs t
        >>= fun v -> add_in_heap ctx ctx.free_addr (Vdval (ctx.free_addr, v)) t
      | CT_PTR tt ->
        add_in_heap ctx ctx.free_addr (Vdval (ctx.free_addr, Vaddress (tt, 0))) t
      | CT_STRUCT tag ->
        get_size t
        >>= fun l ->
        add_in_heap
  ...
  and eval_fn_blck ctx from_main in_fn convt cur_t palcs =
    ...
    | st :: sts ->
      ...
      | Return _ | Break | Continue ->
        get_ret_val new_ctx new_ctx.last_value
        >>= fun lv ->
        rm ctx ctx.pack_addr ctx.free_addr;
        return
          ( { new_ctx with
  ...
  and eval_ifel_blck ctx from_main in_fn rwrt convt cur_t palcs =
    ...
    | st :: sts ->
      ...
      >>= fun (new_ctx, pls) ->
      (match new_ctx.jump_stmt with
      | None -> eval_ifel_blck new_ctx from_main in_fn rwrt convt cur_t pls sts
      | Return _ | Break | Continue ->
        rm ctx ctx.pack_addr ctx.free_addr;
        return
          ( { new_ctx with
  ...
  and eval_fun ctx from_main convt cur_t palcs name vals =
    ...
    let is_void = function
      ...
      | _ -> false
    in
    get_from_functions ctx name
    >>= (function
          | Some f -> return f
          | None -> error "Function undeclared")
    >>= fun (vrrrs, r_typ, args, vs) ->
    if List.length args = List.length vals
    then (
      Hashtbl.iter
        (fun k v ->
          match v with
      ...
      List.fold_left2
        (fun ct a v ->
          ...
          | CARGS (_, n) ->
            ...
            >>= fun ((_, v'), _) ->
            to_type ctx @@ get_val v'
            >>= fun t -> create_var c n (Vhval (Vrval (n, get_val v'))) t)
        (return ct)
        args
        vals
      >>= fun c ->
      ...
      >>= fun (ctxs', pls) ->
      get_ret_val ctxs' ctxs'.last_value
      >>= fun lv ->
      to_type ctx lv
      >>= fun x ->
      match ctxs'.jump_stmt with
      | Return _ ->
        return
          ( ({ ctx with free_addr = ctxs'.free_addr; last_value = lv }, ctxs'.last_value)
          , pls )
      | _ when is_void r_typ ->
        return (({ ctx with free_addr = ctxs'.free_addr }, ctx.last_value), pls)
  ...
  and op (ctxs : exec_ctx) opp e1 e2 from_main in_fn convt cur_t palcs =
    eval_expr ctxs from_main in_fn convt cur_t palcs e1
    >>= fun ((cs, v1), pls) ->
    eval_expr cs from_main in_fn convt cur_t pls e2
    >>= fun ((cts, v2), pls') ->
  ...
  and cast_ks ctx v1 v2 =
    ...
    | Vaddress (tt, _), _ | Varraddress (tt, _), _ ->
      conv_to_addr ctx tt v1
      >>= fun v1' ->
      conv_to_addr ctx tt v2
      >>= fun v2' -> sizeof ctx tt >>= fun k -> return @@ ((1, v1'), (k, v2'))
    | _, Vaddress (tt, _) | _, Varraddress (tt, _) ->
      conv_to_addr ctx tt v1
      >>= fun v1' ->
      conv_to_addr ctx tt v2
      >>= fun v2' -> sizeof ctx tt >>= fun k -> return @@ ((k, v1'), (1, v2'))
    | _ ->
      conv_to_int ctx v1
      >>= fun v1' -> conv_to_int ctx v2 >>= fun v2' -> return ((1, v1'), (1, v2'))
  ...
  and rewrite_var ctxs name t v addr =
    ...
    | CT_ARRAY (l, t) ->
      ...
      | Vvalues _vs ->
        addr_fst
        >>= fun ad ->
        cast_default_init0 ctxs @@ CT_ARRAY (l, t)
        >>= fun dv ->
        return @@ lift_vvs _vs []
        >>= fun vs ->
        (match dv with
          ...
          List.fold_right
            (fun v acc ->
              ...
              | Vstructval (_, v') ->
                ...
                >>= fun ac ->
                to_type ctxs v'
                >>= fun t -> get_size t >>= fun inc -> return @@ (inc :: ac)
              | otherwise ->
                acc
                >>= fun ac ->
                to_type ctxs otherwise
                >>= fun t -> get_size t >>= fun inc -> return @@ (inc :: ac))
            lifted_vs
            (return [])
          >>= fun incs ->
          if List.length incs = List.length lifted_vs
          then
            List.fold_left2
              (fun a _v _inc ->
                a
                ...
                | Vstructval (_, Varraddress _) -> return @@ (a + _inc)
                | _ ->
                  conv_v ctxs _v ov
                  >>= fun v' ->
    ...
    | CT_STRUCT tag ->
      ...
      | Vvalues (Vstaddress (tag', a') :: _vs) ->
        cast_default_init0 ctxs @@ CT_STRUCT tag
        >>= fun dv ->
        return @@ lift_vvs _vs []
        >>= fun vs ->
        (match dv with
        | Vvalues (Vstaddress (_, _) :: dvs)
          ...
          List.fold_right
            (fun v acc ->
              match v with
              | Vstructval (_, v') ->
                acc >>= fun ac -> to_type ctxs v' >>= fun t -> return @@ (get_size t :: ac)
              | _ -> acc)
            lifted_vs
            (return [])
          >>= fun incs ->
            ...
            List.fold_left2
              (fun a _v _inc ->
                ...
                | _ ->
                  conv_v ctxs _v ov
                  >>= fun v' ->
                  Hashtbl.replace ctxs.heap a (Vdval (_ad, Vstructval (nm, v')));
                  return @@ (a + inc))
              (return ad)
              lifted_vs
              incs
          else error "Wrong number of elements"
        | Vvalues (Vstaddress (_, _) :: dvs) when List.length vs = List.length dvs ->
  ...
  and cast_default_struct_init (ctxs : exec_ctx) tag =
    let rec unpack_t (n : string) = function
      ...
      | CT_ARRAY (size, tt) ->
        let rec helper size tt acc =
          if size > 0
          then unpack_t n tt >>= fun ntt' -> helper (size - 1) tt (acc @ ntt')
          else return acc
        in
    ...
    | (_t, _) :: tl ->
      List.fold_right
        ...
        tl
        (return [])
      >>= fun vsts -> cast_default_init _t >>= fun x -> return @@ (x :: vsts)
    | _ -> error "~~"

  and cast_default_init0 ctxs = function
    | CT_INT -> return @@ Vint 0
    | CT_CHAR -> return @@ Vchar 'n'
    | CT_STRUCT n -> cast_default_struct_init ctxs n >>= fun x -> return @@ Vvalues x
  ...
  and get_ret_val ctx x =
    ...
    | Vstaddress (tag, a) ->
      cast_default_init0 ctx @@ CT_STRUCT tag
      >>= (function
      | Vvalues (_ :: vs) ->
        List.fold_right
          (fun v ac -> ac >>= fun ac -> to_type ctx v >>= fun t -> return @@ (t :: ac))
          vs
        @@ return []
        >>= fun ts ->
        List.fold_right
          (fun t ac -> ac >>= fun ac' -> get_size t >>= fun inc -> return @@ (inc :: ac'))
          ts
          (return [])
        >>= fun incs ->
        List.fold_left
          (fun p inc ->
            p
            >>= fun (a', ac) ->
            return @@ (a' + inc, (get_val @@ Vhval (get_from_heap ctx a')) :: ac))
          (return (a, []))
          incs
        >>= fun (_, init) -> return @@ Vvalues (Vstaddress (tag, a) :: List.rev init)
  ...
  and cast_default_arr_init (ctxs : exec_ctx) size tt =
    ...
    let rec helper (acc : v_value list) s t =
      if s > 0
      then cast_default_init0 ctxs t >>= fun xs -> helper (acc @ to_list xs) (s - 1) t
      else return acc
    in
    helper [] size tt
  ...
  and cast_default_ptr_init ctx t =
    let rec destruct acc = function
      ...
      | otherwise ->
        cast_default_init0 ctx otherwise >>= fun v -> return @@ List.rev @@ (v :: acc)
    in
    destruct [] t

  and assign (ctxs : exec_ctx) l_expr r_expr from_main (in_fn : bool) convt cur_t palcs =
    let rec get_ptr_t = function
      | CT_PTR tt -> tt
      | _ -> CT_VOID
    in
    eval_expr ctxs from_main in_fn convt cur_t palcs l_expr
    >>= fun (p, _) ->
    match snd p with
    (* function *)
    | Vhval (Vrval (n, _)) ->
      ...
      >>= fun cs ->
      conv_t cs (get_val r) t
      >>= fun r' ->
      rewrite_var cs n t r' addr
      >>= fun cs' ->
      (match get_ptr_t t with
      | CT_STRUCT tag ->
        get_from_vars cs' n "Var undefined"
        >>= (function
          ...
          | Vdval (aa, vv) when get_val vv <> Vnull ->
            ...
            >>= (function
                  | _, a, _ ->
                    ...
                    return ())
            >>= fun _ ->
            Hashtbl.replace cs'.heap aa (Vdval (aa, Vstaddress (tag, aa + 4)));
            return (cs', pls)
          | _ -> return (cs', pls))
        | _ -> return (cs', pls))
      | _ -> return (cs', pls))
    | Vhval (Vdval (_ad, _v)) ->
      ...
      | CT_STRUCT _ | CT_ARRAY _ ->
        rewrite_var cs "." _convt r _ad
        >>= fun cs ->
        get_from_vars cs "." "Var undefined"
        >>= (fun (_, addr, _) ->
  ...
  and declare
    ...
    | CT_INT | CT_CHAR ->
      ...
      (match expr with
      | None -> return @@ (h, palcs)
      | Some r -> assign h (VAR_NAME (prfx ^ name)) r from_main in_fn t cur_t palcs)
    | CT_PTR tt ->
      let rec get_ptr_t = function
        | CT_PTR tt -> tt
        | _ -> CT_VOID
    ...
    | CT_STRUCT _ ->
      cast_default_init0 ctxs t
      >>= fun v ->
      create_struct ctxs (prfx ^ name) v t
      >>= fun h ->
      (match expr with
      | None -> return @@ (h, palcs)
      | Some r -> assign h (VAR_NAME (prfx ^ name)) r from_main in_fn t cur_t palcs)
    ...
    >>= fun (s_ctx, pls) ->
    let nahv = get_from_vars s_ctx (prfx ^ name) "Var undefined" >>= fun v -> return v in
    nahv
    >>= fun (t, a, hv) ->
  ...
  and declare_top ctx = function
    | TOP_VAR_DECL (name, t, expr) ->
      ...
      >>= fun (ctx', _) ->
      get_from_vars ctx name "Var undefined"
      >>= (function
      | t, a, Vrval (n, v) ->
  ...
  and create_arr ctxs name vvs = function
    | CT_ARRAY (size, tt) ->
      ...
      | Vvalues vs ->
        let ctx = ctxs in
        let fst_val_addr =
          get_size @@ CT_ARRAY (size, tt) >>= fun s -> return @@ (ctx.free_addr + s)
        in
        fst_val_addr
        >>= fun ad ->
        create_var
  ...
  (* let test1 =
    ...
    @@ TOP_FUNC_DECL
         ...
         , BLOCK
             [ VAR_DECL
                 ("tst", CT_PTR CT_INT, Some (FUNC_CALL ("malloc", [ LITERAL (CINT 16) ])))
             ; VAR_DECL
                 ("tst0", CT_PTR CT_INT, Some (FUNC_CALL ("malloc", [ LITERAL (CINT 1) ])))
             ; EXPRESSION (FUNC_CALL ("free", [ ADD (VAR_NAME "tst", LITERAL (CINT 1)) ]))
         ...
         , BLOCK
             ...
             ; (*WHILE
               ...
               FOR
                 ...
                 , Some (ASSIGN (VAR_NAME "i", ADD (VAR_NAME "i", LITERAL (CINT 1))))
                 , BLOCK
                     [ ASSIGN
               ...
               FOR
                 ...
                 , BLOCK
                     [ FOR
                         ...
                         , Some
                             (ASSIGN (VAR_NAME "j", ADD (VAR_NAME "j", LITERAL (CINT 1))))
                         , BLOCK
  ...
  let test0 =
             ...
             [ IF_ELSE
                 ...
                 , BLOCK
                     [ RETURN
                         (MUL
                            ( VAR_NAME "a"
                            , FUNC_CALL ("ff", [ SUB (VAR_NAME "a", LITERAL (CINT 1)) ])
                            ))
                     ] )
    ...
    @@ TOP_FUNC_DECL
         ...
         , "fy"
         , []
         , BLOCK [ VAR_DECL ("a", CT_STRUCT "node", None); RETURN (VAR_NAME "a") ] )
    >>= fun ctx ->
    declare_struct ctx @@ TOP_STRUCT_DECL ("rot", [ CARGS (CT_STRUCT "node", "value") ])
    >>= fun ctx ->
    declare_fun ctx
    @@ TOP_FUNC_DECL
         ( CT_PTR CT_VOID
         ...
         , BLOCK
             ...
             ; WHILE
                 ...
                 , BLOCK
                     ...
                     ; ASSIGN (VAR_NAME "a", ADD (VAR_NAME "a", LITERAL (CINT 1)))
                     ] )
             ; RETURN (LITERAL (CINT 1))
             ] )
    >>= fun ctx ->
    ...
    @@ TOP_FUNC_DECL
         ...
         , "fyf"
         , []
         , BLOCK [ VAR_DECL ("a", CT_STRUCT "node1", None); RETURN (VAR_NAME "a") ] )
    >>= fun ctx ->
    declare_struct ctx
    @@ TOP_STRUCT_DECL ("list", [ CARGS (CT_PTR (CT_STRUCT "node"), "head") ])
    >>= fun ctx ->
    declare_fun ctx
    @@ TOP_FUNC_DECL (CT_VOID, "vf", [], BLOCK [ VAR_DECL ("a", CT_STRUCT "node1", None) ])
    >>= fun ctx ->
    declare_fun ctx
    @@ TOP_FUNC_DECL
         ( CT_INT
  ...
  let eval_d stmts vrs =
    ...
    let vs =
      List.map
        (fun x ->
          get_from_vars ct x "can't find var"
          >>= function
    ...
    List.fold_left2
      (fun acc n xx ->
        xx >>= fun x -> acc >>= fun ac -> return @@ ac ^ n ^ " ~ " ^ show_v_value x ^ "\n")
      (return "")
      vrs
      vs
  ;;
