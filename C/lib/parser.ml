...
let is_keyword = function
  | "break" | "case" | "continue" | "return" | "else" | "for" | "if" | "while" -> true
  | _ -> false
;;
...
let include_stats =
  space *> string "#include " *> char '<' *> identifier
  >>= fun id -> string ".h" >>= (fun part -> return @@ id ^ part) <* char '>'
;;
...
let char_value = char '\'' *> any_char <* char '\'' >>= fun ch -> return @@ CCHAR ch
...
let cmpop =
  _lte <|> _lt <|> _gte <|> _gt <|> _neq <|> _eq <?> "compare operator expected" <* space
;;
...
let ptr value =
  space *> many (token "*")
  >>= fun num_ptrs -> return (build_ptr (List.length num_ptrs) value)
;;
...
let expr =
  fix (fun expr ->
      ...
      let accesor =
        ...
        accessible
        >>= fun h ->
        many1 @@ (token "." *> accessible)
        >>= fun a -> return @@ List.fold_left access h a
      in
      let arrow =
        indexer_exp
        <|> parens var_name
      ...
      let func_call =
        ...
        identifier
        >>= fun idd ->
        token "(" *> sep_by (token ",") (arg_sizeof <|> expr)
        <* token ")"
      ...
      let defr_op =
        fix (fun defr_op ->
            token "*" *> (parens expr <|> indexer_exp <|> var_name <|> defr_op <|> expr)
            >>= function
...
let struct_decl =
  let tkd =
    ...
    <|> (token_datatypes
        >>= fun tdd -> identifier >>= fun idd -> return @@ CARGS (tdd, idd))
  in
  let content =
    token "{"
    *> skip_while (fun c -> is_whitespace c || is_end_of_line c)
    *> many1
         (tkd <* token ";" <* skip_while (fun c -> is_whitespace c || is_end_of_line c))
...
let struct_initialize =
  fix (fun struct_initialize ->
      token "{" *> space *> sep_by1 (token "," *> space) (expr <|> struct_initialize)
      >>= fun ls_init -> token "}" *> (return @@ INITIALIZER ls_init))
;;

let var_decl =
  let p_array =
    token "[" *> take_while1 is_digit
    <* token "]"
    >>= fun intt -> return @@ int_of_string intt
  in
  let transformate ls =
  ...
  let rec get_value idd = function
    ...
    | CT_ARRAY (len, bt) ->
      ...
      | INITIALIZER inits ->
        return
        @@ VAR_DECL (idd, CT_ARRAY (len, bt), Some (LITERAL (CARRAY (transformate inits))))
  ...
  let rec is_initialized idd t =
    ...
    | Some '=' -> advance 1 *> space *> get_value idd t >>= fun v -> token ";" *> return v
    | Some ';' ->
      advance 1
      *>
      (match t with
      | CT_ARRAY _ -> return (VAR_DECL (idd, t, None (* Some (LITERAL (CARRAY [])) *)))
...
let arg_decl =
  token_datatypes
  <* space
  >>= fun td -> identifier >>= fun idd -> return @@ CARGS (td, idd)
;;

let return_ =
  token "return" *> space *> peek_char
  >>= function
...
let var_assign_proc_call no_ends_semic =
  let rec build_def n acc = if n = 0 then acc else build_def (n - 1) (DEREFERENCE acc) in
  ...
  let assign_left_cons num_ptrs left_cons =
    let assign_op op =
      struct_initialize
      <|> expr
      >>= fun r -> if no_ends_semic then return @@ op r else token ";" *> (return @@ op r)
    in
...
let del_space_newline = take_while (fun c -> is_whitespace c || is_end_of_line c)
...
let func_decl stmtss =
  ...
  space *> identifier
  >>= fun idd ->
  token "(" *> sep_by (token ",") arg_decl
  <* token ")" *> skip_many end_of_line
  >>= fun arg_ls ->
...
let if_stmt stmts =
  token "if" *> token "(" *> expr
  <* token ")"
  >>= fun rel_expr -> block stmts >>= fun blk -> return @@ IF (rel_expr, blk)
;;
...
let if_else_stmts stmtss =
  ...
  block stmtss
  >>= fun blk_if ->
  token "else" *> block stmtss
  >>= fun blk_else -> return @@ IF_ELSE (rel_expr, blk_if, blk_else)
;;

let for_statement stmtss =
  token "for"
  *> token "("
  *> (var_decl >>= (fun var -> return @@ Some var) <|> (return None <* token ";"))
  >>= fun var ->
  expr
  >>= (fun re -> return @@ Some re)
  <|> return None
  >>= fun re ->
  token ";"
  *> (var_assign_proc_call true >>= (fun step -> return @@ Some step) <|> return None)
  >>= fun step ->
  token ")" *> skip_while (fun c -> is_whitespace c || is_end_of_line c) *> block stmtss
...
let while_stmt stmtss =
  token "while" *> token "(" *> expr
  <* token ")"
  >>= fun re -> block stmtss >>= fun blk -> return @@ WHILE (re, blk)
;;

let top_decl_c (retype, name, args, blk) = TOP_FUNC_DECL (retype, name, args, blk)

let multi_includes =
  many1 (include_stats <* end_of_line)
  >>= fun includes_list -> return @@ C_INCLUDE includes_list
;;
...
let top_decl_var =
  var_decl
  >>= function
  | VAR_DECL (idd, typ, right_cons) -> return @@ TOP_VAR_DECL (idd, typ, right_cons)
  | _ -> fail "Not a var"
;;

let prog =
  del_space_newline *> multi_includes
  >>= fun mincl ->
  del_space_newline
