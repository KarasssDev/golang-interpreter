...
type state = {env: exval Env.id_t; context: effval Env.eff_t}
...
let exval_to_str = function
  ...
  | TupleV x ->
      Printf.sprintf "(%s)" (String.concat "," (List.map str_converter x))
  | ListV x ->
      Printf.sprintf "[%s]" (String.concat ";" (List.map str_converter x))
  | FunV (pat, _, _) -> ( match pat with PVar x -> x | _ -> "error" )
  | ContV _ -> "cont"
  | EffV _ -> "eff"
...
let rec match_pat pat var =
  ...
  | (PTuple pats, TupleV vars | PList pats, ListV vars)
    when List.length pats = List.length vars ->
      List.fold_left2
        (fun binds pat var -> binds @ match_pat pat var)
        [] pats vars
  | PConst x, v -> (
    match (x, v) with
...
let rec eval_exp state = function
  | EConst x -> (
    ...
    | CString x -> StringV x )
  | EVar x -> (
    try lookup_in_env x state
    with Not_bound -> failwith "Interpretation error: undef variable." )
  | EOp (op, x, y) ->
  ...
  | EMatch (exp, mathchings) ->
      ...
      let exp_state =
        List.fold_left
          (fun state (id, v) -> extend_context id v state)
          state effh in
          ...
          try
            ...
            let state =
              List.fold_left
                ...
                state binds in
            eval_exp state exp
          with Match_fail -> do_match tl ) in
      do_match mathchings
  | EPerform (Effect eff, exp) -> (
      let (EffectH (pat, cont_val, exph)) =
        try lookup_in_context eff state
        with Not_bound -> failwith "no handler for effect" in
      let lookup =
        try lookup_in_env eff state
        with Not_bound -> failwith "no effect found" in
      match lookup with
      | EffV tp ->
          let binds =
            (cont_val, ContV (tp, eff)) :: match_pat pat (eval_exp state exp)
          in
          let state =
            List.fold_left
              (fun state (id, v) -> extend_env id v state)
  ...
  | EContinue (Continuation cont_val, exp) -> (
      let lookup_cont =
        try lookup_in_env cont_val state
        with Not_bound -> failwith "not a continuation value" in
      match lookup_cont with
      | ContV (tp_cont, eff) -> (
          let lookup_eff =
            try lookup_in_env eff state
            with Not_bound -> failwith "effect not found" in
          match lookup_eff with
          | EffV tp_eff ->
              if tp_cont = tp_eff then eval_exp state exp
              else failwith "wrong cont"
          | _ -> failwith "internal error" )
      | _ -> failwith "internal error" )
...
let eval_test decls expected =
  try
    ...
    let state =
      List.fold_left (fun state decl -> eval_dec state decl) init_state decls
    in
    let res =
      IdMap.fold
        (fun k v ln ->
          ...
          new_res )
        state.env "" in
    if res = expected then true else (Printf.printf "%s" res; false)
  with Tuple_compare ->
    if
      expected
...
let%test _ =
  eval_test
    [ DLet
        (false, PVar "x", ELet ([(false, PVar "y", EConst (CInt 5))], EVar "y"))
    ]
    "x -> 5 "
...
let%test _ =
  eval_test
    [ DLet
        ...
        , PVar "x"
        , ELet
            ( [ (false, PVar "y", EConst (CInt 5))
              ; (false, PVar "z", EConst (CInt 10)) ]
...
let%test _ =
  eval_test
    [ DLet
        ( false
        , PVar "f"
        , EFun (PVar "x", EFun (PVar "y", EOp (Add, EVar "x", EVar "y"))) ) ]
    "f -> x "
...
let%test _ =
  eval_test
    ...
    ; DLet
        ( false
        , PVar "a"
        , EApp (EApp (EVar "f", EConst (CInt 1)), EConst (CInt 2)) ) ]
    "a -> 3 f -> x "
...
let%test _ =
  eval_test
                        ...
                        , [ ( PCons (PVar "hd1", PCons (PVar "hd2", PVar "tl"))
                            , EIf
                                ...
                                , ECons
                                    ...
                                    , EApp
                                        ( EVar "sort"
                                        , ECons (EVar "hd1", EVar "tl") ) )
                                , ECons
                                    ( EVar "hd1"
                                    , EApp
                                        ( EVar "sort"
                                        , ECons (EVar "hd2", EVar "tl") ) ) ) )
                          ; (PVar "tl", EVar "tl") ] ) ) ]
                , EIf
    ...
    ; DLet
        ( false
        , PVar "l"
        , EList [EConst (CInt 1); EConst (CInt 3); EConst (CInt 2)] )
