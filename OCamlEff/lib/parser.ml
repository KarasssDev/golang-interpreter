...
let procr op pl pr =
  let p =
    fix
    @@ fun p ->
    pl >>= fun l -> op >>= (fun op -> p <|> pr >>| op l) <|> return l in
  p
...
let is_keyword = function
  | "let" | "in" | "true" | "false" | "rec" | "fun" | "perform" | "if"
   |"continue" | "else" | "then" | "with" | "effect" | "function" | "match" ->
      true
  | _ -> false
...
let app_unop p =
  choice
    [ token "-" *> p >>| eunop Minus; kwd "not" *> p >>| eunop Not
    ; token "+" *> p; p ]
...
let pack =
  ...
  let key d =
    ...
    let elet =
      ...
      in
      trim @@ lift2 elet (many1 binding) (d.exp d) in
  ...
  let op d =
    ...
    let prim =
      trim
      @@ choice
           [ lst >>| elist; uns_const >>| econst; ident >>| evar
           ; parens @@ d.exp d ] in
    ...
    trim @@ procl disj conj_op @@ d.key d in
  {key; tuple; exp; op}
...
let tyexp =
  ...
  let prim =
    ...
    @@ choice
         ...
         ; (uns >>| fun bind -> TVar (int_of_string bind)); parens tyexp ] in
  let list =
    let* lst_ty = prim in
    let* l = many1 @@ (empty *> token "list") in
    let rec wrap acc n =
      match n with 0 -> acc | _ -> wrap (TList acc) (n - 1) in
    return @@ wrap lst_ty (List.length l) in
  let tup =
...
let decl =
  let dlet =
    lift3 dlet
      (kwd "let" *> option false (kwd "rec" >>| fun _ -> true))
      pat
      (lift2 efun (empty *> many pat <* token "=") exp) in
