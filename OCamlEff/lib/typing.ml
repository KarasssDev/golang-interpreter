...
let rec infer context = function
  ...
  | EOp (op, exp1, exp2) ->
    ...
    | _, expr, _ ->
      let* subst, t = infer context expr in
  ...
  (* | EList exps ->
    (match exps with
    | hd :: tl ->
      let* s1, t1 = infer context hd in
      let kek =
        List.fold_left
          (fun (subst, tyexp) exp ->
            let* s, t = infer context exp in
            let* s_uni = unify t tyexp in
            Subst.(s ++ s_uni ++ s1 ++ subst), Subst.apply s_uni t)
          (Subst.empty, t1)
          tl
      in
      return kek
