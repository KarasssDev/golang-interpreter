...
1. Java и generics (2 человека)
   * целые числа, рекурсия, стандартные конструкции языка
   * присваивание, анонимные функции
   * классы, интерфейсы, наследование
   * а также более точное указание generic параметров (например, `<? super x>` и т.п.)
      * если заработает проверка типов и интерпретатор на вот такой программе, то будет круто
      ```
      interface Z {}
      interface N<x> {}
      interface L<x> {}
...
##### Определюсь/допишу потом если тем будет не хватать/или кому-то очень захочется
